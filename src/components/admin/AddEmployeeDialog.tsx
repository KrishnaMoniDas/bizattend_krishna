...
async function scanRfidTag(): Promise<string | null> {
  // Reduced simulation delay (300-800ms instead of 500-1500ms)
  await new Promise((resolve) =>
    setTimeout(resolve, Math.random() * 500 + 300),
  );

  if (Math.random() > 0.1) {
    // 90% success rate instead of 80%
...
export function AddEmployeeDialog({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Unexpected error during employee addition:", e);
      toast({
        title: "Error Adding Employee",
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg glass-effect">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the details for the new employee. Fields marked with{" "}
